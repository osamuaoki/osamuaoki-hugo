...
まず、FIRMWAREはAT90USB1286とほぼ同じコードなんで、以下の`rules.mk`設定で、何とかコンパイルして一見書き込むまではいきました。
...
...
```
実行ログからは、DFUを使って、ファームウエアーが0x08000000から書き込まれています。
...
でもファームウエアーが書き込まれたはずなのに、USBコネクターを抜き差ししても、USBデバイス自身がHIDとして認識されません。
...
RESETとBOOT0の両押しを離しDFUを起動した時には、ちゃんとデバイスとして繋がるから、壊れてはいません。
