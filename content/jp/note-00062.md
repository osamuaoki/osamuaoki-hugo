...
2026-02-27: 確認すると、マイナンバーサイトからのリンクでXML形式ででもダウンロードできます。

```console
 $ xmllint --format FILENAME.XML > FILENAME-ML.XML
```
こうすると、見やすく整形できます。

さらにXML形式は[直接LibreOffice Calcに読み込めます](https://help.libreoffice.org/latest/en-US/text/scalc/01/xml_source.html)。
...
(2026-02-27: 自分へのメモ)この後、かなり改変していて今上記は使わずfbankです。個人情報が多いのでPOST しません。
