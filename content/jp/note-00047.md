...
そこで、上記のデーターはそのままで別に左右のアルファベットの間隔を更に広げ、カーソールキーに真ん中で直接アクセスできるキーマップも用意して、どちらかというとこちらを今使ってってきています。
...
確かに指は長さが違うので、縦スタガートに興味がありました。

胸の前に自然に手を持ってきて手首に一番ストレスが無いようにキーボード上に置くと,意外とキーボードを押す際のホームポジションは意外と一直線の位置でした。

実際小指を置くキーの位置を１つ下げて試すと、すべての手の指はまっすぐ前に向くのですが手首が内側になり無理がかかり不快でした。

## PCB参考情報

片側`3x6_3`キー構成のスプリットの小さな携帯タイプが面白そうなので、当分は取り組まないけどPCBを作る際に参考となる情報をまとめました。
