...
この「サポート」の件は無視します。あくまで「これ以外で使用禁止」などはないので、間抜けな制約は、クライアントのブラウザーでの対応で回避すろことを考えます。今年からは「WINDOWS11」が要件なので巷に転がっていたプラグインでの簡易対応ができません。
...
* 「スマートフォンを使用する」をクリックしてPOPUP
  * 「ご利用のパソコン環境（OS/ブラウザ）は、QRコードを利用したマイナンバーカード方式によるe-Taxの推奨バージョン（国税庁において動作を確認した環境）ではありません。」との高言をいただくお役所からいただく。このままPOPUPは閉めない。右下のボタンは「閉じる」
