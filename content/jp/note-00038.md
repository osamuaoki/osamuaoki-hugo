...
メモリーがタイトなatmega32u4は上記のnanoBootが良いと思いますが、AT90USB1286は余裕があるので
qmkにあるLUFA改のLEDブリンク付で充分ではないでしょうか?（メモリーが大きいのとPLLが違うのでnanoBootの移植に挫折しました。）

https://github.com/qmk/qmk_firmware/tree/master/util
...
PE2 (ALE, /HWBE)は、TEENSY系基板では1Kで接地されている。
...
USB経由でのファームウエアの更新の際はプロンプトがあり、RESETを押しブートローダーを起動するように要求がありますます。

もちろん、PC側でターミナルを開き、`journalctl -f` か `sudo dmesg --follow` 等としてPCのカーネル挙動を観察しブートローダーとして立ち上がったデバイス認識の確認という対応でもできます。

ブートローダーがここで作ったLED対応版だとLEDがつくことでUSBデバイスが認識されたことが簡単に分かります。意外とコネクタの接触不良もあり、このLED機能良いです.
