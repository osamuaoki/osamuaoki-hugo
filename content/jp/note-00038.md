...
メモリーがタイトなatmega32u4は上記のnanoBootが良いと思いますが、AT90USB1286は余裕があるので選択肢があります。でも一筋縄ではない。
...
MCUの基本確認事項の、BOOTのモードの状況を確認します。

TEENSY系の基板のPE2 (ALE,/HWBE)は、TEENSY系基板では1Kで接地されています。だから、BOOT挙動は以下です。
...
さらに、TEENSY 2.0 (atmega32u4) には`./halfkey-update...`コマンドで、、TEENSY 2.0++ (at90usb1286) には、`avrdude -c dfu`コマンドか`dfu-programmer`コマンドで、blink系のプログラムを導入しUSB経由でのブートローダーの動作検証をします。
（うまくいかない場合は、ISP経由での導入でボードが壊れてないか確認します。）

DFUにはATmel専用みたいで古くからある`dfu-programmer`以外に、DFU 1.0 and DFU 1.1
specifications対応でSTM32対応している`dfu-util`、
どうも一番新しい`fwupd`に入っていてDFU 1.0, DFU 1.1 and the ST DfuSe vendor
extension,対応している`dfu-tool`があります。この辺は後ほど確認します。
...
* https://manpages.debian.org/testing/fwupd/dfu-tool.1.en.html
