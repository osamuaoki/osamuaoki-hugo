...
プログラムメモリーのアドレッシングはワード（2 BYTE）単位です。だから直接
ジャンプ(JMP, CALL)やデーターアクセス(LPM, SPM)も、アッセンブラーでが
問題なく128KBまで簡単に対応できます。

つまりATmega1284P やAT90USB1287などもOKです。

GCCにAVRオプションに関するところには；
...
ただCでは64KB以上ではヘッダーファイルを見る限り３２ビットポインター
（内部処理は２４ビットとか？）の`_PF`関数が要るようなことがあるよう
なので使い方の詳細は要確認です。
...
* [AVR106: C Functions for Reading and Writing to Flash Memory](http://ww1.microchip.com/downloads/en/Appnotes/Atmel-2575-C-Functions-for-Reading-and-Writing-to-Flash-Memory_ApplicationNote_AVR106.pdf)
