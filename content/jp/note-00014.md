...
ISPプログラマーのAVRISP mkII を用い、まずFUSEを初期化（ロック解除）し、
さらに`optiboot_atmega328.hex`をフラッシュメモリーに書き込むことで
Optiboot プログラムのインストールができます。

```
 $ sudo avrdude -p m328p -c avrisp2 -e -F -U lock:w:0x3F:m \
                -U efuse:w:0xfd:m -U hfuse:w:0xde:m -U lfuse:w:0xff:m
...
* [簡単！ATmega328Pにブートローダを書きこ](http://blog-yama.a-quest.com/?eid=970124)
