...
ただ、これでも`Makefile`の起動コマンドは分かりますが、`Makefile`から実行されたコマンドが何かが確認できません。実は以下が`Makefile`の先頭にかかれているのがこの現員です。
...
シェルから以下を実行すると、`Makefile`から実行されるコマンドが分かります。
...
dfu-util -d 0483:DF11 -a 0 -s 0x08000000:leave -D .build/cgg56_default.bin
dfu-util 0.9
