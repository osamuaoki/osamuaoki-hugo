...
結線に意外と使い勝手が良かったのは意外でした。メスデュポンと
Jフック型端子のコネクター線を今度の機会に作りたくなりました。
...
のサイズとブートリセットアドレスで決めます。$0000に置く
ファームウエアーが、ブートローダーに後で飛べるようになっていない
普通の場合はブートリセットアドレス=$0000を非設定にすべきです。

4KBの標準的なブートローダーだと:

* HFUSE: 0xD8 (ブートリセットアドレス=$0000　設定 DFU - KBD系)
* HFUSE: 0xD9 (ブートリセットアドレス=$0000　非設定　Caterina系)

512BのnanoBoot/teensyブートローダーだと:

* HFUSE: 0xDF (ブートリセットアドレス=$0000　非設定 - teensy)
