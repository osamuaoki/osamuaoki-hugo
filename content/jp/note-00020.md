...
離した際にはbootloaderが始動する」というのが基本です。
...
が、「リセットボタンを押して離した際にbootloaderが始動」しますが、
...
ケーションのファームウエアーが動く」ようです。リセット時にbootloader
が動くのはTeensyと同じですが、Arduino系のbootloaderが賢く条件を満たせ
ばアプリケーションのファームウエアーがリセットされて動くようになって
います。
...
起動関係を読んでいて一体どうして割り込みVector表をプログラムするのか
が最初見えなかった。

割り込みルーチンは`ISR(<割り込みVector名>)`という名前の関数と
して定義します。

リンカーがコンパイル時に、この関数にあわせて、スタートアップコード中
の割り込みVector表に、この割り込みルーチンのアドレスをいれて
割り込みVector表を完成するらしいと分かって、やっと分かった気が
しました。だから、`blink.lst`には見当たらないが、`blink.lss`等に
詳しく残っています。
