...
チップのデフォルトはBOOTRST非設定(リセット=1)、HWBE設定(セット=0)
となっています。
...
Boot Reset Fuse (BOOTRST) は、設定(セット=0)すると、ハード
リセットがかかるとブートローダーに飛びますが、非設定(リセット=1)
だと、アプリのファームウエアーのアドレス$0000に飛びます。

ただ、Boot Reset Fuse (BOOTRST) が非設定(リセット=1)でも、
ALE/HWB pinが接地されているteensyやPromicroやの場合HWBE FUSE
ビットが設定(セット=0)されているとハードリセットがかかると
有無言わずブートローダーに飛びます。

ポイントは普通の開発ボードでHWBE FUSEビットが設定(セット=0)
しているとBoot Reset Fuse (BOOTRST)は関係ないということです。

もちろん、HWBE FUSEビット非設定(リセット=1)で、Boot Reset Fuse
(BOOTRST)設定(セット=0)して、$0000に置くファームウエアーに
ハードリセット時に飛ばすのはかなり特殊です。$0000に置く
