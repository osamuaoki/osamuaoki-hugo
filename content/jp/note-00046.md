...
`dfu-util`のアップストリームのgit sourceに含まれる情報で、現在WEB上で読める[Using dfu-util with DfuSe devices](http://dfu-util.sourceforge.net/dfuse.html)には　STM32F4のようなDfuSeデバイスの使い方の説明がありました。(残念ながら、Tarされたソースや、Debianのパッケージにはこの情報は現在(2022-03)含まれていません。)
...
DfuSeデバイス用のファイルフォーマットが`.dfu`拡張子が付いたファイルで、`dfuse-pack.py`を用いて作成できる、いくつかのメモリーブロックが組み合わさったファイルだそうです。`dfuse-pack.py`はDebianでは`dfuse-pack`で、manpageはまだ無かったです。
