...
ヘッダーで使われるハードウエアーに対応するレジスター名やビットの名前を最近
のより強力なAVRと共通にしたためか「AVR Libc Reference Manual」にあるデモ
プロジェクトのコードはそのままでは動かなくなってました。使うレジスター名
等がデーターシートと微妙に異なっていたのでヘッダーファイルの定義を見比べて、
微妙に数字を加えたりの合わせ込みをしました。
...
をするとホストPCに端子の接地状況を含めた文字列が印字されました。
