...
シリアルに「Hello」等を出力する簡単なプログラムの`hello.c`を作成しました。
（シリアル出力なしで、デモコードより簡単です。）
...
#define BAUD 115200
#define BAUD_TOL 3
// Usable Baud Rate 300 600 1200 2400 4800 9600 14400 19200 28800 38400 57600
...
int main(void) {
  ...
  while (1) {
    if ((previous == SURE_HI) & (check() == SURE_LO)) {
      // LED ON
      PORTB |= BIT5HI;
      printf_P(PSTR("Hello world! PINB = %02X\n"), PINB);
      // LED OFF
      PORTB &= ~BIT5HI;
      previous = SURE_LO;
...
`avr-libc`が提供するヘッダーで使われるハードウエアーに対応するレジスター名
やビットの名前を、最近のより同一機能レジスターが複数ある強力なAVRと共通に
したためか、「AVR Libc Reference Manual」にあるデモプロジェクトの
コードはそのままでは動かなくなってました。ヘッダーファイルの定義と
見比べて、レジスター名等に微妙に数字を加えたりの合わせ込みをしました。
...
をするとターミナルプログラムを起動したホストPCに、端子の接地状況を含め
た文字列が印字されます。
