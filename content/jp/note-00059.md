...
ただ、数学的なところはしっかりしているようだが、会計処理が分かっていれば使えるのかもしれませんが、素人にわかりやすい実務的な期首・期末の売掛・買掛処理などややこしい機能へのWIIZARDのようなサポート対応はない感じです。

期間指定してのデーターファイルの切り出し保存処理他も簡単ではないようです。ただ、GNUCASHの保存ファイルはXMLなので、PYTHONでスクリプト書くなどすればかなりフレキシブルな感じです。
