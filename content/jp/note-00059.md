...
よくB/Sと略される貸借対照表(Balance sheet)は、GnuCash(日本語)では、`帳票`->`資産・負債`と進んだ先に、`貸借対照表`があり、GnuCash(英語)では、`Reports`->`Asset & Liabilities`と進んだ先に、`Balance sheet`があり、選択すると表示されます。会計用語はわかりにくいので、日英表記を交えておさらいします。
