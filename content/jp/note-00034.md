...
ext4にしていたルートファイルシステムもbtrfs化します。

実際にはデュアルブートで別のルートファイルシステムを使いながら、
通常のルートファイルシステムをマウントせず`btrfs-convert`でこれを処理します。

ファイルシステムの変換自体はは、
[Conversion from Ext3/4 and ReiserFS](https://btrfs.wiki.kernel.org/index.php/Conversion_from_Ext3)
をトレースしました。

ただこれだけでは、ブート可能なシステムにはなっていません。
...
##   ルートファイルシステムのsubvolumeへの移動

timeshift等を使うには、ルートファイルシステムをsubvolume`@`へ移動する必要があります。
こうすると、マルチブートの他のシステムからの起動が少々とリッキーです。
...
UUID=??? /home/ btrfs subvol=@home,user_subvol_rm_allowed,defaults    0    0
```

## No_COW attribute

No_COWアトリビュートをディレクトリー毎`chattr +C <dirname>`で設定できるようです。
