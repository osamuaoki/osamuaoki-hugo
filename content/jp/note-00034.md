...
## Busterリリース後の d-i で btrfs への システム導入

busterリリース版のd-iでbtrfsのpartitionを作成し、それをroot filesystem
に指定すると、`@rootfs`というsubvolumeが作成され、その中にシステムが導入されます。

ホームディレクトリーにあるユーザーデーターを保護する上でも、ホームディレクトリー
は別のsubvolumeに保存するのが良いようです。

## 過去のRoot-fsのbtrfs化トリック(今は無効)

RAID関係、圧縮関係、SWAP FILE関係に嵌らない限りbtrfsはかなり安定なようなので
ext4にしていたルートファイルシステムも含めてbtrfs化をめざします。

以下に示すように事をしてroot
filesystemを`@`に置くと、2022年７月以降はトラブルが発生しました。素直に上記の様にd-i
で btrfs への root filesystem導入刷るのが良いようです。
