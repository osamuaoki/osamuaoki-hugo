...
Proton-C以外のARM系ボードでは、USB経由のDFUでファームウエアーを書き換えるには、「BOOT1-push」「RESET-push」「RESET-release」「BOOT1-releas」と操作するので、ボード上の２つのPUSHボタンへのアクセスが必要。
代替策は、SWD経由アクセス。
<!-- 電源投入時にBOOT0押す？-->
