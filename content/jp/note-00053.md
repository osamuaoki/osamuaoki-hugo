...
## キーボードのTAP-HOLDタイミング問題


キーボードのTAP-HOLDタイミングは、端のキーのimmediateタイプとは違うdelayedタイプがHOME-ROW-MODでは必要。、

 * immediate: immediate hold (capslock)
 * delayed:   immediate pending state for the key (wait for B↑ or A→) (homerow)
