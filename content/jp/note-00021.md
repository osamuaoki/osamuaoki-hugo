...
には、マニュアルでデモ`debian/gbp.conf`を用いるとともに、`git-config`を
使い調整することに触れられています。
...
こんな場合でも、 アップストリームとパッケージングのレポからの
チェックアウトを共用ローカルディレクトリーにすると、`gitk`で
`upstream/master`にある最新のアップストリーム変更からパッチを
cherry-pickして使えるので便利です。そのためだけなら、まず
過去のパッケージ履歴を含むパッケージ用のGitレポの作成を、
`gbp import-descs ...`等で行い、更に
`git remote add upstream upstream_url`(この`upstream`はremotes名)
してリモート名を登録、ここで`git fetch -p upstream`すれば
アップストリームのレポ中の`master`以外のブランチをfetchせず、また
ローカルのレポのブランチ名との競合を気にせず`gitk --all`でアクセス
できるアップストリームのレポ履歴がローカルに落とせます。履歴は
繋がってませんがcherry-pickするだけならこれで充分です。

こうしてできた履歴に、`gitk --all`でアクセスしてローカルブランチ名を
適当にGUIで設定すれば、ローカルチェックアウトもできます。

ちなみに、どこにも繋がらない履歴をブランチ名`orphan_branch_name`で
フレッシュスタートするのは、`git checkout --orphan orphan_branch_name`
です。(前記の`git fetch`の代わりに、`git pull`系コマンドを用いるには
この準備が必要。その際ローカルのブランチ名との競合には対策が必要で
面倒くさい。)
