...
## デフォールトのブランチ名の「`main`」への移行

２１世紀の世情の反映で、 [PC](https://en.wikipedia.org/wiki/Political_correctness)
の視点から、デフォールトのブランチ名は、「`master`」に代わり、新規のプロジェクトでは
「`main`」が使われていきます。

そういった意味で、上記は「`master`」を「`main`」と読み替えていく必要があります。

そのために、GithubのWeb interfaceのSettings -> Branches -> Default branch で
...
これだけでは、リモートのトラッキングがうまくできません。そこで追加の
関連設定をします。
...
これで新ブランチ名で使えるのですが、`remotes/origin/master`が残っているのが
