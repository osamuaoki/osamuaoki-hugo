...
他の人に迷惑で、初歩的ミスが晒され格好も悪いし、コードをレビュー
しにくしてしまいます。

作業中履歴はローカルもコミットだけにして、`git rebase`して綺麗に
したあとの履歴をリモートの公開共有レポにアップするのが普通かと理解
しています。ただそれだと作業途中のローカルを失うと全て無くなって
しまいます。もちろんローカルのバックアップが残っていれば良い
タイミングでされていれば良いのですが、それを保証するのは難しい
面があります。またHDDやSSDというものはいつ死んでも不思議はない
ので、同一デバイス上のスナップショット記録等は当てにすべきでは
ありません。

そんな場合`master`にコミットする代わりに作業用のブランチとして
例えば`wip`を作成し作業中履歴をそこにコミットし`git push`す
る回避策が考えられます。ただそれでも他人に見られるので格好は良
くないことは否めません。
...
ブランチ名を変えたくない場合の簡単な解決法は、リモートの変更です。
USBメモリー等に作った非公開レポをリモートにします。
...
るようになっています。またうっかり公開レポに`git push`しないよ
うに、安全錠を掛けています。
...
$ git push --all
```
上記のように作業中を進め、要点毎に非公開レポにも記録します。さらに
念のため公開レポの`master`の最新内容に`git rebase`して同期していき
ます。作業結果が満足となった時点で、安全錠を外して公開レポに綺麗な
形で`git push`します。
...
ここで、２回行った`git remote set-url ...`操作は間違ってリモートの
公開共有レポにアップするのを防ぐオプショナルな操作で必須ではありませ
ん。ただ`git version 2.26.2`では`--delete`操作が少々トリッキーでした。
単純に`.git/config`ファイルの`pushurl`設定行をエディターで直接消す方
が確実な気がしました。
...
とくあります。この場合は上記で行ったようにアップストリームのGitレポ
のヒストリーとうまく繋がりません。その場合、`gbp import-descs ...`
等をしてパッケージ用のGitレポを作成し、その中に
`git checkout --orphan upstream`としてアップストリームのGitレポを
`git fetch -p upstream`出来るようにし、最新アップストリーム変更から
`gitk`等でパッチをcherry-pickして使うのも有効な方策かもしれません。
