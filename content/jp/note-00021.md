...
のが普通かと理解しています。ただこれだと、何らかに事故で作業途中の
ローカルデーターを失うと、全ての最近の作業結果が無くなってしま
...
そんな場合、共有の`master`にコミットする代わりに作業用のブランチ
...
ブランチ名を変えたくない場合の簡単な作業中履歴法は、リモートの
変更です。例えば、USBメモリー等に作った非公開レポをリモートに
します。
