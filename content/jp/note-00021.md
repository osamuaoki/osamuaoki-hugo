...
Gitを使う際に、自分が作ったフォークされたレポを、アップストリーム
のレポの更新に合わせて如何にスムーズに更新をしていくかは、
レポの開始方法やブランチ名の設定アプローチやコマンド実行手順に
解がいくつもあるだけに悩まされます。

よく混乱していつも悩むので、備忘録を兼ねて、アップストリームがGITHUBを使っている
場合を例に以下にメモを作成します。
...
* `upstream`という名前をつけたリモートへのpushの禁止
    ...
    * 上記操作で`.git/config`ファイルに「`pushurl = DISABLED_FOR_PUSH`」
    というラインができている。
    この行頭に`#`を挿入しコメントアウトするとこの設定を無効化できます。
