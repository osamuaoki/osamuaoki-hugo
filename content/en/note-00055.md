...
気をつけなければいけないのは、コンテナ側のファイル名は「コンテナ名/フルパス」の書式であるということです。sampleコンテナの/etc/environmentなら「sample/etc/environment」になります。「⁠-r」オプションをつけると、再帰的に受け渡しします。またcpコマンドなどと同じく送信元のファイル（＝最後の引数を除いたファイル）は複数指定可能です。その場合、保存先はディレクトリになります。

lxc file pushの場合、「⁠-p」オプションを付けることで保存先のパスに合わせてディレクトリを作成します。また「--uid」「⁠--gid」「⁠--mode」オプションによって保存先のファイルオーナーやモードを変更できます。
