...
I usually install non-Desktop system first and set up GNOME Desktop after
successful install.

When `task-desktop` is added via `aptitude` to support GUI Desktop, let's
comment-out settings for "The primary network interface" in
`/etc/network/interfaces` to enable full control of network connections by the
Network Manager.
