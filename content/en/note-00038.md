...
* Set-up ssh server (optional)
  ...
  * Copy ssh public keys of this machine into controlling machine's `~/.ssh/authorized_keys`
  * Disable password-based login via `sudo vi /etc/ssh/sshd_config` to set `PasswordAuthentication no`.
  * Copy ssh public keys of the controlling machine into this machine's `~/.ssh/authorized_keys` (optional)
