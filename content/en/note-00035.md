...
The recent Windows ISO image contains `sources/install.wim` which is about 5GiB and exceeds 4 GiB filesize limit on FAT32 frequently used for the USB flash disk.
...
- Copy everything in the ISO image to USB flash drive
  - split `sources/install.wim` using `wimsplit`, or equivalently `wimlib-imagex split`, into parts

If your UEFI supports NTFS, you can skip splitting `sources/install.wim`.
(I read this tip somewhere before but I haven't test this.)
