...
* fzf and fzf.vim with rg
* gitgutter
* vim-python-matchit (I use locally patched 0.5)
* vim-airline
* vim-airline-themes

Please note some basic configuration settings including plugin management
including `~/.vim/vimrc` are at
[dot-vim](https://github.com/osamuaoki/dot-vim/blob/master/vimrc).


I now set up those IDE-like heavy features as on-demand and low profile. (You
can still disable package by comment out corresponding `packadd!` line with
leading `"`.)  
