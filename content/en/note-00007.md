...
* fzf and fzf.vim with rg (with `<leader>`)
* gitgutter (on demand)
* vim-python-matchit (I use locally patched 0.5)
* vim-airline
* vim-airline-themes

Please note actual basic configuration settings including plugin management
including `~/.vim/vimrc` are at
[dot-vim](https://github.com/osamuaoki/dot-vim/blob/master/vimrc) which goes
with my dotfiles:
[osamu-utils](https://github.com/osamuaoki/osamu-utils).  No sensitive info's
are in these.  That's backed up locally.

I now set up those IDE-like heavy features as on-demand and low profile. (You
can further disable package by comment out corresponding `packadd!` line with
leading `"`.)
...
* Less keyboard customization is better.  For now, I set them up as:
    * `Q`: Paste MACRO
    * `<C-t>`: swap chars[-2],[-1]
    * `<leader><leader>`: Highlight search word under the cursor
        * `<C-L>` to clear
        * `#` to replace
    ...
    * `<leader>a`: Toggle ALE
    * `<leader>B`: BTags selection (FZF.VIM)
    * `<leader>b`: Buffers selection (FZF.VIM)
    * `<leader>c`: Color selection (FZF.VIM)
    * `<leader>i`: Toggle GitGutter
    * `<leader>f`: Files selection (FZF.VIM)
    * `<leader>G`: GFiles selection (FZF.VIM)
    * `<leader>g`: GFiles? selection (FZF.VIM)
    * `<leader>hp`: preview hunk (FZF.VIM)
    * `<leader>hu`: undo hunk (FZF.VIM)
    * `<leader>l`: Vim line indent formatter
    * `<leader>m`: Maps selection (FZF.VIM)
    * `<leader>r`: Rg search selection (FZF.VIM)
    * `<leader>s`: Strip tailing spaces
...
I set the GNOME terminal's font to [Hack](https://tracker.debian.org/pkg/fonts-hack).
Not all non-standard `powerline`fonts are included.  So skip them in `airline`
This has easy-to-identify `0oO` and `1lI` fonts and the cute  non-standard
glyphs used for producing cute bar with airline..
...
buffers, text search, ... .  I also installed `ripgrep`, too.  Nice window
menus.

`ripgrep`(`rg`) and `ugrep`(`ug`) seems to be SMP aware and 10x faster than
`ag`.  Since `rg` is Git aware (`.gitignore`), this was my choice.
...
Setting to '`Hack 14`' required to use escaped space.

## Quote and comment and line continuation in Vim

* Quoted string in VimScript is single quote.
* Comment lines start with double qupote
* Line continuation is reverse sladh prepended to the following line.  It seems
