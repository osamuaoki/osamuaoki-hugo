...
again, I feel like I need to cut back on some fancy packages more.
...
For now, I set them up as:
  ...
  * `<leader><leader>`: Highlight search word under the cursor in NORMAL, or exact selected word in VISUAL to "-register
      ...
      * `<leader>sa` to create substite-all template
  ...
  * `<leader>ss`: Rotate SPELL and SYNTAX mode
...
`<leader>sa` so this is more than '#' and '*'.
...
These are safer alternatives to avoid such incidences.
