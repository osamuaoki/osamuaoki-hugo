...
To send mails with "@debian.org" address, I use ssh+sendmail (created shell
script `~/bin/sendmail-p.d.o`).  I use this from Evolution, `devscripts`, and
`reportbug` whenever I wish to send out mails with "@debian.org" address.

I usually don't bother with fetchmail these days.

## Command line

* `hal`: https://github.com/osamuaoki/osamu-utils
* `vim`: https://github.com/osamuaoki/dot-vim
* `fzf`: nice way to customize shell and vim.
* `hardlink`
