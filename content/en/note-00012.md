...
NOTE: Permission of `/var/cache/apt-cacher-ng/` is `drwxr-sr-x` with `apt-cacher-ng:apt-cacher-ng`.  Erasing its contents reset broken caching data related issues.
...
* `/etc/schroot/desktop-osamu/` containing copy of `/etc/schroot/desktop/`
  contents customized for use by `schroot-shell`.
