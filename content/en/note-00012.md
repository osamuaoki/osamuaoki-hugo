...
better to have all required packages installed.

### Creating pre-loaded chroot environments for schroot

Let's create package pre-loaded chroot environments which can run x-apps using
the schroot directly using a configuration based on `/etc/schroot/desktop/`.
(My user name is "osamu" and my hostname is "goofy".)
...
SYNOPSIS
    schroot-shell [|--init|--prep|--upgrade] [DIST]

DESCRIPTION
    This is modeled after *sbuild-shell* and *sbuild-update* commands.

    A customized schroot profile based on '/etc/schroot/desktop/*' needs
    to be created.  Currently '/etc/schroot/term/*' profile
    is used.  This default profile can be changed to '/etc/schroot/foo/*'
    by setting '${SCHROOT_PROFILE}' environment variable to 'foo'.

    This normally starts persistent (source) chroot environment for the
    DIST distribution and provides a shell prompt in the current console
    terminal with access to the data in the current home directory.
    This is modeled after *sbuild-shell*.

    If DIST is unset, '${DIST}' environment variable is used instead.
    If '${DIST}' environment variable is also unset, 'unstable' is used.

    DIST can be 'unstable', 'testing', 'stable', 'oldstable', or 'oldoldstable'.
    DIST can be their shortforms, i.e., 'u', 't', 's', 'o', or 'oo'.

    If '--init' or '-i' is given as an option, this initializes schroot
    environment for this command and optionally for sbuild.
      * '/srv/chroot/${DIST}-amd64-sbuild' used by sbuild as a minimum system.
      * '/srv/chroot/${DIST}-amd64-term' used by this program to
        start the terminal shell prompt as a source chroot of a loaded system.
    This is a wrapper around *sbuild-createchroot*.

    If '--prep' or '-p' is given as an option, this updates schroot
    environment with basic extra packages normally needed for the console
    terminal operations to '/srv/chroot/${DIST}-amd64-term'

    If '--upgrade' or '-u' is given as an option, this updates schroot
    environment of '/srv/chroot/${DIST}-amd64-term'
    This is modeled after *sbuild-update*.
```

I further customize mount points by editing my custom configuration
`/etc/schroot/term/fstab` based on `/etc/schroot/desktop/fstab`
