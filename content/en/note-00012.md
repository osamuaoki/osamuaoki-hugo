...
Here, I follow https://wiki.debian.org/sbuild .
...
Let's create the configuration file `~/.sbuildrc` as:
```
cat >~/.sbuildrc << 'EOF'
$build_arch_all = 1;
...
EOF
cat /usr/share/doc/sbuild/examples/example.sbuildrc >> ~/.sbuildrc
```
...
Since I have enough DRAM and want to avoid were out of SSD, I added
...
```
$ sbuild --source-only-changes
```

or using source package as:

```
$ sbuild --source-only-changes <packagename>.dsc
```

Here, the `--source-only-changes` flag enables to obtain a `.changes` file
suitable for the source-only upload required by recent Debian.
...
You can login to the baseline chroot environment as:

```
$ sudo sbuild-shell source:unstable-amd64-sbuild
```
