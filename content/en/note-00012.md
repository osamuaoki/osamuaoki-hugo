...
each test build as sbuild does is time consuming.  Let's create a package
pre-loaded unstable chroot environment which can run x-apps using the schroot
directly.
...
I set `dev` as the alias to get into the test build environment from anywhere
in my home directory. (My user name is "osamu")

```
$ echo 'alias dev="xhost +si:localuser:osamu && schroot -c unstable-dev" && xhost -' >> ~/.bash_aliases
$ . .bashrc
```
...
```
$ dev
localuser:osamu being added to access control list
(unstable-dev)$ sudo apt update
 ...
(unstable-dev)$ exit
exit
...
I also copied setting from the host PC for sudo to avoid password prompt and
add a few symlinks in `/root` as in the host PC.

To run x-apps such as `gitk` inside chroot, please start schroot as:
