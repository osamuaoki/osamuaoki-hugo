...
(I was suffering sudden cowbuilder failure. That was another motivation.)

Planned infrastructure includes:

* APT proxy on host WS.
* Avahi mDNS environment with domain nane "local"
* sbuild infrastructure for chroot package build.
* VM as KVM for package build. lternative(non-GUI)
* VM as KVM for package testing. (GUI)
* SSH connection among VM systems.
...
Basically, followed https://wiki.debian.org/sbuild .
...
Add few lines somewhere except last in `~/.sbuildrc`.
```
$build_arch_all = 1;
...
`/etc/schroot/chroot.d/unstable-amd64-sbuild-$suffix` to speed up.
```
command-prefix=eatmydata
```

Please be aware, `/srv/chroot` and `/var/lib/sbuild` may eat up some disk
spaces.
