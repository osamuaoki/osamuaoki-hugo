...
* https://github.com/osamuaoki/osamu-utils/blob/main/distsh

```
$ distsh h
NAME
    distsh - start a shell in chroot with current home data etc.

SYNOPSIS
    distsh [|init|prep|upgrade]

DESCRIPTION
    Start persistent (source) chroot with schroot with
    current home data based on the ${DIST} environment.
    If DIST is unset, it defaults to 'unstable'

    If 'init' is given as an argument, this initialize schroot
    environment for this command and sbuild.
      * /srv/chroot/${DIST}-amd64-sbuild used by sbuild as a minimum system
      * /srv/chroot/${DIST}-amd64-term used by this program to start
        terminal shell prompt as a source chroot of a loaded system

    If 'prep' is given as an argument, this update schroot
    environment with basic extra packages normally needed in
    /srv/chroot/${DIST}-amd64-term

    If 'upgrade' is given as an argument, this update schroot
    environment
```
...
* Explicitly bind mount subdirectories under my home directory which is on
  different device (including Btrfs subvolume).
* Bind mount /root

### Easy access to the schroot environment

In order to access the unstable chroot environment, I simply type.

```
$ distsh
```

In order to update the unstable chroot environment, I simply type.

```
$ distsh upgrade
```

In order to access the oldstable chroot environment, I simply type.

```
$ DIST=oldstable distsh
```

In order to update the oldstable chroot environment, I simply type.

```
$ DIST=oldstable distsh upgrade
```

For updating unstable chroot used by sbuild, use the standard command.

```
$ sbuild-update -udcar unstable
```

For updating oldstable chroot used by sbuild, use the standard command.

```
$ sbuild-update -udcar oldstable
```
