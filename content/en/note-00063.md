...
NOTE: The real root cause of issue was the existance of FF codes in the GPL2 LICENSE file.
      See https://github.com/pypa/sampleproject/issues/209
...
I replaced `LICENSE.txt` with GPL LICENSE file content from `imediff` and test
...
This is ERROR on `unexpected unindent` and WARNING.
...
I was suspicious about TeX style quotations in GPL license text: `\`...'`
and removed them as first check.
...
Still, this is ERROR on `unexpected unindent` and WARNING.

Following ERROR message, I removed indentation of leading spaces and tabs and got:
...
Now this has no ERROR (just WARNING).  Good enough.
...
This is again ERROR on different type `start-string without end-string` and WARNING.

Simply commenting out `license = "LICENSE.txt"` line in `pyproject.toml`
cleanly fixes as:
...
```console
 ...
 ...
Successfully built sampleproject_osamu_debian_org-3.2.0.tar.gz and sampleproject_osamu_debian_org-3.2.0-py3-none-any.whl
Checking dist/sampleproject_osamu_debian_org-3.2.0-py3-none-any.whl: PASSED
Checking dist/sampleproject_osamu_debian_org-3.2.0.tar.gz: PASSED
```

Resulting `METADATA` doesn't contain copy of "LICENSE.txt", of course.
