...
So I can't type `CTRL-F` to start NORMAL-MODE-like COMMAND HISTORY editing.

(I guess I need to set up 'cedit' option to either `CTRL-O` or `CTRL-X` if I
want this feature.)
<!--
Unused CTRL-chars in NORMAL MODE are:
CTRL-O
CTRL-X
CTRL-S    (Screen freeze: but no)
CTRL-Z    (Sleep: but no)
CTRL-@    (NULL: no)
