...
For `v`, and `nvim`, *neovim* is used on my system.  (`v` is accessed via bash alias)

For `vi` and `vim`, *vim* is used on my system.  (`vi` is accessed via `update-alternatives`).

I also create bash aliases `vim0` and `vi0` to start `vim` and `nvim` with `-u NORC`.  These are handy when editor configuration files contain bugs.
