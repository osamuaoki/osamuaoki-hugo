...
the `-g` option.  See <<_compile_hello_gdb,ELF: Compile hello-gdb>>.
...
stored in the ELF file as described in <<_readelf_wl,ELF: readelf -wL>> to
...
option in <<_compile_hello_gdb,ELF: Compile hello-gdb>> to GDB.
...
<<_assembler_code, GCC: Assembler code>>.  Its value is set to 0x40067c. 
...
The <<_libthread,Library: libthread>> had a buggy `prime8.c` code.  Let's check what was wrong under GDB.

<!-- Here I cheat since I had prime8.xxx as prime8.c in the above. -->

Let's compile  `prime8.c` with `-g` as `prime8-gdb` and run it to create the `core` file as <<_core_dump,ELF: Core dump>>, then analyze it with GDB.
