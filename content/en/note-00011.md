...
### Evolution workaround

Evolution for Debian 11 Bullseye under wayland has ugly font problem as of May
2, 2021.  (Some firefox, too).  It looks like some WebKit bug exposed by
sandboxing. https://gitlab.gnome.org/GNOME/evolution/-/issues/1161

Since my PATH lists `/usr/local/bin` before `/usr/bin`, I workaround this
problem by creating a shell script `/usr/local/bin/evolution`.

```
#!/bin/sh
WEBKIT_FORCE_SANDBOX=0 exec evolution "$@"
```

See https://bugs.debian.org/975039
