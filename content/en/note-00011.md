...
tidy-up package auto install flags (`M`) missing for some libraries.

Use of `~i!~prequired!~pimportant!~pstandard!~M` in `l` prompt in `aptitude` to
...
The `gnome-tweak` package installed as a depencdency of `gnome` package helps to tweak the system.
...
### Evolution workaround (font) -- not needed
