...
### Autoformat as opt-in feature

I want autoformat as opt-in feature.

Since `opts.autoformat` is initialized in
`LazyVim/lua/lazyvim/plugins/lsp/init.lua` as `true`, I decided to override it by creating
`~/.config/nvim/lua/plugins/lsp.lua` by setting `opts.autoformat` to `false`
