...
* toggle completion (`nvim-cmp`, Toggle with `<leader>uP`)
* enable auto-input (with `<leader>ua`) by completion, auto pairs, auto format
* disable auto-input (with `<leader>uA`) by completion, auto pairs, auto format
