...
See [newer updated guide]({{< ref note-00080.md>}}) for `Trixie`.
...
* On a system where `Control-SPACE` is usable (My Thinkpad `Bookworm` and `Trixie`; and my Sony Vaio
`Trixie`)
  * SHIFT-SPACE: 全角かな入力 (hiragana mode) 変換キー equivalent (activate IM)
  * CTRL-SPACE: 英数半角入力 (latin mode) 無変換キー equivalent (deactivate IM)
* On a system where `Control-SPACE` seemed not usable (My Sony Vaio `Bookworm`.)
  * SHIFT-SPACE:  漢字 (kanji mode) キー equivalent (toggle IM) as work around
...
### Editor setups
See [Re-learning Vim (7)]({{< ref "note-00040.md" >}}).

* Main editor: `nvim` with `~/.config/nvim` (alias `nv`)
  * NeoVim 0.10.  See [Building Neovim]({{< ref "note-00039.md" >}}).
...
See [HEIF image files]({{< ref "note-00041.md" >}}) on more HEFI tips.
