...
Here is a YAML data example (`wayland.yaml`) to add `disk` device for
...
NOTE: I don't know if lock file as above works well or not.
...
## Running Xwayland GUI application `xeyes` in container
...
## Running X GUI application `xeys` in container
