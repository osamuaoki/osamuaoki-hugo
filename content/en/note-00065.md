...
Let's learn to use playbook via `ansible-playbook` command first with APT operation.
...
Since I added `tags:` keys in the previous example, I can selectively play a
part of playbook.  For example:
...
If your working directory is still placed on `/path/to/ansible-config-data/`
